...
public static class ImageUtilities
{
    ...
    /// <param name="path">The texture path on the device</param>
    public static bool SaveTextureToPath(Texture2D texture, string path)
    {
    ...
    public static Texture2D SaveTexture(Texture2D texture, string name)
    {
        string path = Application.persistentDataPath + "/" + name + ".png";

        if (SaveTextureToPath(texture, path))
            return GetSavedTexture(name);
        else
            return null;
    }

    public static Texture2D GetSavedTexture(string textureName)
    {
